...
- **Tarjeta de tabla sin título dentro.** La `<Card>` que contiene una `<Table>` nunca lleva `<CardHeader>`/`<CardTitle>`: arranca directo en `<CardContent className="p-0">` con el `<TableHeader>` de la tabla. Si la sección necesita título, descripción, filtros o cifras resumen, van en una `<Card>` aparte (solo `<CardContent>`, sin `CardHeader`) inmediatamente arriba — patrón ya usado en `tutor-view.tsx`, `dashboard.tsx` y `tutor-detail.tsx`.
